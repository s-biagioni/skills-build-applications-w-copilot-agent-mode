    ...
    <div className="container mt-4">
      <div className="card">
        ...
        <div className="card-body p-0">
          {filtered.length === 0 ? (
            <div className="p-3">No activities found.</div>
          ) : (
              ...
              <table className="table table-hover table-striped mb-0">
                ...
                <tbody>
                  {filtered.map((it, idx) => (
                    <tr key={it.id || idx}>
                      {columns.map(col => (
                        <td key={col}>{formatValue(it[col])}</td>
                      ))}
                      <td>
                        <button className="btn btn-sm btn-outline-primary me-2" onClick={() => { setSelected(it); setShowModal(true); }}>Details</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <Modal title="Activity Details" show={showModal} onClose={() => setShowModal(false)}>
